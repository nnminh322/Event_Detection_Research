...
class MAVENPreprocess(object):
    ...
    def __init__(self, root, feature_root, tokenizer, l2v, label_start_offset=1, max_length=512, expand_context=False, split_valid=True):
        ...
        train_instances = self._file(train_file, "train")
        valid_instances = self._file(valid_file, "valid")
        test_instances = self._file(test_file, "test")
    ...
    def _file(self, file_path, split:str):
        ...
        with open(file_path, "rt") as fp:
            for document_line in tqdm(fp):
                ...
                instances.extend(self._document(document, split))
        self.clear_sentences()
        return instances
...
def main():
    ...
    m1 = MAVENPreprocess(MAVEN_PATH, feature_root, tokenizer=bt, l2v=l2v)
