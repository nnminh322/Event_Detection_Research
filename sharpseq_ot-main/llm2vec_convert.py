...
import random as rd
import h5py
from llm2vec_convert import LLM2Vec
...
class MAVENPreprocess(object):
    ...
    def __init__(self, root, feature_root, tokenizer, l2v, label_start_offset=1, max_length=512, expand_context=False, split_valid=True):
        super().__init__()
        self.out = h5py.File(os.path.join(root, "data.h5"), "w")
        self.out.close()
        train_file = os.path.join(root, "train.jsonl")
        valid_file = os.path.join(root, "dev.jsonl")
        ...
        train_instances = self._file("data/MAVEN/train.origin", "train")
        valid_instances = self._file("data/MAVEN/dev.origin", "valid")
        test_instances = self._file("data/MAVEN/test.origin", "test")
        # with open("data/MAVEN/train.origin", "wt") as fp:
        #     for instance in train_instances:
        #         fp.write(json.dumps(instance.todict())+"\n")

        # with open("data/MAVEN/dev.origin", "wt") as fp:
        #     for instance in valid_instances:
        #         fp.write(json.dumps(instance.todict())+"\n")
        
        # with open("data/MAVEN/test.origin", "wt") as fp:
    ...
    def clear_sentences(self,):
        ...
        with torch.no_grad():
            ...
            #         self.model = transformers.BertModel.from_pretrained("./bert-large-cased").to(device)
            #     else:
            #         self.model = transformers.BertModel.from_pretrained("bert-large-cased").to(device)
            #     self.model.eval()
    ...
    def _file(self, file_path, split:str):
        ...
        with open(file_path, "rt") as fp:
            for document_line in tqdm(fp):
                ...
                instances.extend(document)
                self.add_sentence(document["sentence_id"], document["piece_ids"])
        self.clear_sentences()
        return instances

    def _document(self, document, split:str):
...
def main():
    ...
    #l2v = LLM2Vec(model, bt, pooling_mode="mean", max_length=512)
    m1 = MAVENPreprocess(MAVEN_PATH, feature_root, tokenizer=bt, l2v=model)
